  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => { setIsAdding(!isAdding); setEditingRateId(null); setFormData({ route: "", provider: "", class: "", basePrice: "", markupPrice: "" }); }} className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground font-semibold rounded-md shadow-sm hover:bg-primary-hover transition-colors">
      ...
      {isAdding && (
        <form onSubmit={handleSubmit} className="bg-card p-6 border rounded-xl shadow-sm grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2 flex justify-end gap-2 mt-4">
             <button type="button" onClick={() => { setIsAdding(false); setEditingRateId(null); }} className="px-4 py-2 border border-slate-300 text-slate-700 rounded hover:bg-slate-50 font-semibold transition-colors">Cancel</button>
             <button type="submit" className="px-4 py-2 bg-primary text-primary-foreground font-semibold rounded shadow-sm hover:bg-primary-hover transition-colors">{editingRateId ? "Update Rate" : "Save Rate"}</button>
          </div>
        </form>
      )}
              ...
              rates.map((rate) => (
                <tr key={rate.id} className="border-b last:border-0 hover:bg-muted/20">
                  ...
                  <td className="p-4 flex gap-2">
                    <button onClick={() => openEdit(rate)} className="text-primary hover:text-primary-hover font-medium">
