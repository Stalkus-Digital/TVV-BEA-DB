...
export class AuthService extends BaseService {
  ...
  async refresh(rawRefreshToken: string, requestContext: RequestContext): Promise<Result<LoginResult, AppError>> {
    ...
    if (token.revokedAt) {
      // Reuse of a rotated-out token — treat as theft: kill the whole session.
   ...
   * Architecture, and none was requested this sprint) — the raw reset
   ...
   * a real user; see docs/22's Remaining TODOs — wiring a real email send
  ...
  async requestPasswordReset(input: unknown): Promise<Result<void, AppError>> {
    ...
    if (userResult.value) {
      ...
      if (isErr(created)) return created;
      this.logger.info("Password reset requested — token generated (no email provider wired, logging for dev use only)", {
        userId: userResult.value.id,
    ...
    // email matched a real account — never leak which emails are registered.
    return ok(undefined);
  }
